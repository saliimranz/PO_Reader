#!/usr/bin/env python3
"""
Refined PDF Parser that correctly extracts items with proper validation
"""
...
class RefinedPdfParser:
    ...
    def _extract_details_data(self, lines: List[str]) -> List[Dict[str, Any]]:
        ...
        # Process each item with better validation
        for start_idx in item_starts:
            detail = self._parse_single_item_refined(lines, start_idx)
            if detail and self._is_valid_item_detail_refined(detail):
    ...
    def _parse_single_item_refined(self, lines: List[str], start_idx: int) -> Optional[Dict[str, Any]]:
        """
        Parse a single item starting at start_idx with refined logic
        """
        ...
        for i in range(start_idx + 1, min(start_idx + 8, len(lines))):
            ...
            if any(skip_word in line.upper() for skip_word in ['PAGE', 'DISCOUNT', 'NET PRICE', 'AMOUNT', 'GRAND TOTAL', 'PURCHASE ORDER', 'TERMS AND CONDITIONS']):
                break
    ...
    def _is_valid_item_detail_refined(self, detail: Dict[str, Any]) -> bool:
        """
        Check if a detail object is valid with refined criteria
        """
        ...
        if detail.get('Description'):
            ...
            if any(skip_word in desc for skip_word in ['PAGE', 'DISCOUNT', 'NET PRICE', 'AMOUNT', 'GRAND TOTAL', 'PURCHASE ORDER', 'TERMS AND CONDITIONS', 'UAE DIRHAM']):
                return False
...
def main():
    ...
    """
    if len(sys.argv) != 2:
        print("Usage: python3 RefinedPdfParser.py <pdf_path>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    try:
        parser = RefinedPdfParser()
