#!/usr/bin/env python3
"""
Production PDF Parser - Final version with high accuracy
"""
...
class ProductionPdfParser:
    ...
    def _normalize_text(self, text: str) -> str:
        ...
        # Normalize multiple spaces within lines (but preserve newlines)
        lines = text.split('\n')
        normalized_lines = []
        for line in lines:
            # Normalize multiple spaces within each line
    ...
    def _extract_details_data(self, lines: List[str]) -> List[Dict[str, Any]]:
        ...
        for start_idx in item_starts:
            detail = self._parse_single_item_production(lines, start_idx)
            if detail and self._is_valid_item_detail_production(detail):
    ...
    def _parse_single_item_production(self, lines: List[str], start_idx: int) -> Optional[Dict[str, Any]]:
        """
        Parse a single item starting at start_idx with production-level validation
        """
        ...
        for i in range(start_idx + 1, min(start_idx + 8, len(lines))):
            ...
            # Skip if it contains common non-item text
            if any(skip_word in line.upper() for skip_word in ['PAGE', 'DISCOUNT', 'NET PRICE', 'AMOUNT', 'GRAND TOTAL', 'PURCHASE ORDER', 'TERMS AND CONDITIONS', 'UAE DIRHAM', 'TRADING FZE']):
                break
    ...
    def _is_valid_item_detail_production(self, detail: Dict[str, Any]) -> bool:
        """
        Check if a detail object is valid with production-level criteria
        """
        # Must have at least item code or description
        if not detail.get('ItemCode') and not detail.get('Description'):
            return False
        
        # Description should not be too long (likely not a real item)
        if detail.get('Description') and len(detail.get('Description', '')) > 200:
            return False
        
        # Description should not contain common non-item text
        if detail.get('Description'):
            desc = detail.get('Description', '').upper()
            if any(skip_word in desc for skip_word in ['PAGE', 'DISCOUNT', 'NET PRICE', 'AMOUNT', 'GRAND TOTAL', 'PURCHASE ORDER', 'TERMS AND CONDITIONS', 'UAE DIRHAM', 'TRADING FZE', 'NINETY-FIVE', 'THOUSAND', 'HUNDRED']):
                return False
        
        # Description should not be just numbers
        if detail.get('Description') and re.match(r'^[\d,\.\s]+$', detail.get('Description', '')):
            return False
        ...
        # If we have an item code, it should look like a real part number
        if detail.get('ItemCode'):
            item_code = detail.get('ItemCode', '')
            # Should contain letters and numbers, not just numbers
            if not re.search(r'[A-Z]', item_code):
                return False
...
def main():
    ...
    """
    if len(sys.argv) != 2:
        print("Usage: python3 ProductionPdfParser.py <pdf_path>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    try:
        parser = ProductionPdfParser()
